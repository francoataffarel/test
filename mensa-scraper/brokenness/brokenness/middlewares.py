...
from .spiders.uitilies.constants import custom_fake_user_agents

logger = logging.getLogger(__name__)

class BrokennessSpiderMiddleware:
...
class BrokennessDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
class BotMiddleware(RetryUserAgentMiddleware):
    ...
    def _get_bot_middleware(url):
        ...
        elif 'amazon.ae' in url:
            return AmazonAeMiddleware()
        elif 'flipkart.com' in url:
            return FlipkartBotMiddleware()
        elif 'myntra.com' or 'nykaa.com' or 'nykaafashion.com' in url:
...
class AmazonAeMiddleware(MarketplaceBotMiddleware):
    def is_bot_discovered(self, response) -> bool:
        bot_container = response.selector.css('.a-last').xpath('./text()').get()
        # TODO: Use arabic text to double check if the page is bot
        if bot_container:
            return True
        return False
