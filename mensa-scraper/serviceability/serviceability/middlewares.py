...
from scrapy_fake_useragent.middleware import RetryUserAgentMiddleware, RandomUserAgentBase, RandomUserAgentMiddleware
from twisted.internet import reactor
from twisted.internet.defer import Deferred

from .spiders.base import MarketplaceBotMiddleware
from .spiders.utils.constants import custom_fake_user_agents

logger = logging.getLogger(__name__)


class ServiceabilitySpiderMiddleware:
...
class ServiceabilityDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
class CustomRandomUserAgentMiddleware(RandomUserAgentMiddleware):
    def process_request(self, request, spider):
        if 'myntra.com' in request.url:
            ...
            request.headers['User-Agent'] = random.choice(custom_fake_user_agents)
        elif 'nykaafashion.com' in request.url:
...
class BotMiddleware(RetryUserAgentMiddleware):
    ...
    def _get_bot_middleware(url):
        ...
        elif 'myntra.com' or 'nykaa.com' or 'nykaafashion.com' or 'ajio.com' in url:
            return ApiBotMiddleware()
