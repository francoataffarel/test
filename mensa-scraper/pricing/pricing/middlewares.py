...
class PricingDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
...
class DelayedRequestsMiddleware(object):
    def process_request(self, request, spider):
        ...
        if delay:
            logger.info(f"Deferred the request by {delay} secs - {url}")
            d = Deferred()
...
class BotMiddleware(RetryUserAgentMiddleware):
    ...
    def process_response(self, request, response, spider):
        url = request.meta['splash']['args']['url'] if 'splash' in request.meta else request.url
        bot = self._get_bot_middleware(url)
        if bot.is_bot_discovered(response):
            logger.info(f"This BOT got discovered, Will retry again - {url}")
            reason = 'bot_detected'
            # # Added a parameter to relay request. The actual delay will be handle by DelayedRequestsMiddleware
