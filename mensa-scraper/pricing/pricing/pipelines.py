...
class S3Pipeline:
    ...
    def __init__(self, settings, stats):
        ...
        s3_pricing_url = settings['S3_PRICING_URL']
        self.pricing_bucket_name = urlparse(s3_pricing_url).hostname
        self.pricing_object_key = urlparse(s3_pricing_url).path[1:]  # Remove the first '/'
    ...
    def process_item(self, item, spider):
        ...
        self.pricing_items.append(item)
        if len(self.pricing_items) >= self.max_chunk_size:
            self._upload_chunk()
        return item
    ...
    def _upload_chunk(self):
        ...
        f = self._make_obj(self.pricing_items)

        # Build object key by replacing variables in object key template.
        brand = ItemAdapter(self.pricing_items[0]).get('brand').lower()
        channel = ItemAdapter(self.pricing_items[0]).get('marketplace')
        channel = get_channel_short_name(channel)
        brand = re.sub(r'\W+', '', brand)
        params = {'chunk': self.pricing_chunk_number, 'time': self.ts, 'name': f"{brand}-{channel}"}
        object_key = self.pricing_object_key.format(**params)
        try:
            self.uploader.upload_file_obj(f, self.pricing_bucket_name, object_key)
        except UploadError:
            ...
            self.pricing_chunk_number += len(self.pricing_items)
            self.pricing_items = []
