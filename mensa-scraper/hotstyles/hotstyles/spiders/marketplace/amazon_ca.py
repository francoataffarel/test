...
def build_style_list_urls(sku_ids: List[str]) -> List[str]:
    return [f'https://www.amazon.ca/dp/{skuid}/' for skuid in sku_ids]


class AmazonCa(AbstractMarketplace):
    smart_proxy = None
    ...
    @property
    def logger(self):
        logger = logging.getLogger(ChannelType.AmazonCa.name)
    ...
    def extract_style(self, response, **kwargs):
        ...
        mrp = clean_amazon_price_value(
            selection.xpath(".//span[text()='M.R.P.: ']//span[@class='a-offscreen']/text()|.//span[@data-a-strike]//span[@class='a-offscreen']/text()").get())
        price = clean_amazon_price_value(
            selection.xpath(".//span[contains(concat(' ',normalize-space(@class),' '),' priceToPay ')]"
        ...
        yield HotstylesItem(skuid=skuid, name=name, channel=ChannelType.AmazonCa,
