...
def build_style_list_urls(sku_ids: List[str]) -> List[str]:
    return [f'https://www.amazon.ae/dp/{skuid}/' for skuid in sku_ids]
...
class AmazonAe(AbstractMarketplace):
    ...
    @property
    def logger(self):
        logger = logging.getLogger(ChannelType.AmazonAe.name)
    ...
    def extract_style(self, response, **kwargs):
        ...
        yield HotstylesItem(skuid=skuid, name=name, channel=ChannelType.AmazonAe,
