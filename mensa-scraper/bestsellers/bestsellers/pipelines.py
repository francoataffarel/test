...
class S3Pipeline:
    ...
    def __init__(self, settings, stats):
        ...
        s3_bestsellers_url = settings['S3_BESTSELLERS_URL']
        self.bucket_name = urlparse(s3_bestsellers_url).hostname
        self.object_key = urlparse(s3_bestsellers_url).path[1:]  # Remove the first '/'
    ...
    def _upload_chunk(self):
        ...
        brand = ItemAdapter(self.items[0]).get('brand')
        channel = get_channel_short_name(channel)
        params = {'chunk': self.item_chunk_number, 'time': self.ts, 'name': f"{brand}-{channel}"}
