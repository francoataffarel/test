...
from ..utils import get_query_param, has_query_param, replace_query_param, ScrapeMode, \
    ChannelType, clean_amazon_price_value
from ...items import StyleItem, StyleReviewItem


class AmazonInStyleList(AbstractStyleList):
...
class AmazonInStyle(AbstractStyle):
    ...
    def clean(self, selection, **kwargs) -> StyleItem:
        ...
        if style['mrp'] == '0':
            ...
            discount = str(int(style['mrp']) - int(style['price']))
        ...
        for star in rating_star_list:
            # Match for two numeric group. Ex: 19% of reviews have 3 stars
        ...
        reviews_href = f"https://www.amazon.in{reviews_href}"
        return StyleItem(
            ...
            best_seller_rank=az_rank, available=available, manufacturer=manufacturer, channel=ChannelType.AmazonIn
        )


class AmazonInStyleReviews(AbstractStyleReviews):
    ...
    def clean(self, selection, **kwargs) -> List[StyleReviewItem]:
        ...
        for review_container in selection:
            ...
            asin = get_query_param(review_href, 'ASIN') if has_query_param(review_href, 'ASIN') else ''
            ...
            style_review_item = StyleReviewItem(id=id, sku=asin, brand=kwargs['brand'],
                                                ...
                                                channel=ChannelType.AmazonIn, extracted_date=extracted_date,
...
class AmazonIn(MarketPlaceFactory):
    ...
    def create_style(self) -> AmazonInStyle:
        return AmazonInStyle()

    def create_style_reviews(self) -> AmazonInStyleReviews:
        return AmazonInStyleReviews()
    ...
    def extract_style(self, response, **kwargs):
        ...
        if self.scrape_products:
            yield self.request(url=f'https://www.amazon.in/hz/reviews-render/ajax/lazy-widgets/stream?asin={style["sku"]}&language=en_IN&lazyWidget=cr-summarization-attributes',
