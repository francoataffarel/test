...
class AmazonAeStyleList(AbstractStyleList):
    ...
    def clean(self, selection, **kwargs) -> List[dict]:
        ...
        for i in range(len(selection)):
            ...
            if asin_product_href.match(product_href):
                product_href = f"https://www.amazon.ae{product_href}"
...
class AmazonAeStyle(AbstractStyle):
    ...
    def clean(self, selection, **kwargs) -> StyleItem:
        ...
        reviews_href = f"https://www.amazon.ae{reviews_href}"
        return StyleItem(
            ...
            best_seller_rank=az_rank, available=available, manufacturer=manufacturer, channel=ChannelType.AmazonAe
        )


class AmazonAeStyleReviews(AbstractStyleReviews):
...
class AmazonAe(MarketPlaceFactory):
    ...
    def create_style_list(self) -> AmazonAeStyleList:
        return AmazonAeStyleList()

    def create_style(self) -> AmazonAeStyle:
        return AmazonAeStyle()

    def create_style_reviews(self) -> AmazonAeStyleReviews:
        return AmazonAeStyleReviews()
    ...
    def extract_style(self, response, **kwargs):
        ...
        for selector in self.create_style().selectors():
            selection = selection.selector.xpath(selector)
        style = kwargs['style']    
        style = self.create_style().clean(selection, url=response.url, brand=self.brand, style=style)

        if self.scrape_products:
            yield self.request(url=f'https://www.amazon.ae/hz/reviews-render/ajax/lazy-widgets/stream?asin={style["sku"]}&language=en_AE&lazyWidget=cr-summarization-attributes',
