...
class AmazonCaStyleList(AbstractStyleList):
    ...
    def clean(self, selection, **kwargs) -> List[dict]:
        ...
        for i in range(len(selection)):
            ...
            if asin_product_href.match(product_href):
                product_href = f"https://www.amazon.ca{product_href}"
...
class AmazonCaStyle(AbstractStyle):
    ...
    def clean(self, selection, **kwargs) -> StyleItem:
        ...
        for star in rating_star_list:
            # Match for two numeric group. Ex: 2% of reviews have 3 stars
            match = re.search('(^[0-9]{1,3})([a-zA-Z% ]*)([0-9]{1})', star)
            # # Add groups in map. If group is not found then by default it will be 0
            rating_star[int(match.group(3))] = int(match.group(1))
        ...
        reviews_href = f"https://www.amazon.ca{reviews_href}"
        return StyleItem(
            ...
            best_seller_rank=az_rank, available=available, manufacturer=manufacturer, channel=ChannelType.AmazonCa
        )


class AmazonCaStyleReviews(AbstractStyleReviews):
...
class AmazonCa(MarketPlaceFactory):
    ...
    def create_style_list(self) -> AmazonCaStyleList:
        return AmazonCaStyleList()

    def create_style(self) -> AmazonCaStyle:
        return AmazonCaStyle()

    def create_style_reviews(self) -> AmazonCaStyleReviews:
        return AmazonCaStyleReviews()
    ...
    def extract_style(self, response, **kwargs):
        ...
        if self.scrape_products:
            yield self.request(url=f'https://www.amazon.ca/hz/reviews-render/ajax/lazy-widgets/stream?asin={style["sku"]}&language=en_IN&lazyWidget=cr-summarization-attributes',
