...
class CloudWatchStatsCollector(StatsCollector):
    ...
    def _persist_stats(self, stats, spider):
        ...
        mode = spider.mode.name
        proxy = str(spider.proxy)
        review_capture_duration = str(spider.review_capture_duration)
        env = config('ENV', default='dev')

        namespace = 'RNR Scraper'
        dimensions = [
            {'Name': 'BRAND', 'Value': brand},
            {'Name': 'MARKETPLACE', 'Value': marketplace},
            {'Name': 'MODE', 'Value': mode},
            {'Name': 'PROXY', 'Value': proxy},
            {'Name': 'REVIEW_CAPTURE_DURATION', 'Value': review_capture_duration},
            {'Name': 'ENV', 'Value': env}
        ]
