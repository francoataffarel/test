...
args = getResolvedOptions(sys.argv, ['GLUE_DB', 'GLUE_SERVICE_TABLE', 'GLUE_RDS_DB', 'GLUE_RDS_TABLE', 'JOB_NAME', 'ROLE_ARN', 'REDSHIFT_DB'])

GLUE_DB = args['GLUE_DB']
GLUE_SERVICE_TABLE = args['GLUE_SERVICE_TABLE']
...
print(f"Reading data from {GLUE_SERVICE_TABLE}")
...
input_df = input_df.withColumn('delivery_date', f.to_date('delivery_date', 'yyyyMMdd'))


input_rds_df = input_rds_df.select('pincode', 'city', 'region')
input_rds_df.dropDuplicates(['pincode'])
print("Location Config Mapping Table")
input_rds_df.show(5)

input_df = input_df.join(input_rds_df, "pincode")

input_df = input_df.select('product_href', 'product_name', 'pincode', 'city', 'region', 'skuId', 'extracted_date', 'delivery_date', 'days_for_delivery',
                           'brand', 'competitor_of_brands', 'marketplace')
...
glueContext.write_dynamic_frame.from_jdbc_conf(
    ...
    connection_options={
        "dbtable": "brand_analytics.serviceability_product",
